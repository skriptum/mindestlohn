...
```{r}
model_foreign <- lm(
    afd_prozent ~ lohn_prozent + ost+ arbeitslosenquote + log(gdp)+ age + foreigners, 
    data = wahl_lohn
  )
...
```{r}
model_pop <- lm(
    afd_prozent ~ lohn_prozent + ost+ arbeitslosenquote + log(gdp) +age+ foreigners + log(pop), 
    data = wahl_lohn
  )
summary(model_pop)
```
...
```
Seems, best model is:
"afd_prozent ~ lohn_prozent + ost + age + arbeitslosenquote + foreigners"
